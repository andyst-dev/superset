...
import { createTestRuntime } from "../db/testDb";
...
describe("readSince", () => {
	...
	test("returns only events after the cursor", () => {
		...
		seed(runtime, 5);
		const replay = readSince(runtime.queries, SESSION, {
			epoch: opened.epoch,
			seq: 3,
		});
	...
	test("signals session_not_found for an unknown session", () => {
		expect(readSince(runtime.queries, "nope", { epoch: "e", seq: 0 })).toEqual({
			ok: false,
	...
	test("signals epoch_changed for a stale-epoch cursor", () => {
		...
		seed(runtime, 2);
		expect(
			readSince(runtime.queries, SESSION, { epoch: "stale-epoch", seq: 1 }),
	...
	test("signals invalid_cursor for a cursor ahead of the journal", () => {
		...
		seed(runtime, 2);
		expect(
			readSince(runtime.queries, SESSION, { epoch: opened.epoch, seq: 9 }),
	...
	test("signals journal_missing when the epoch's rows are gone", () => {
		const opened = runtime.journal.open(init());
		seed(runtime, 3);
		runtime.db.exec("DELETE FROM chat_journal");
		expect(
			readSince(runtime.queries, SESSION, { epoch: opened.epoch, seq: 2 }),
	...
	test("a reopened session whose journal was lost mints a new epoch", () => {
		...
		const reopened = new ChatJournal(runtime.queries).open(init());
		expect(reopened.epoch).not.toBe(opened.epoch);
		expect(reopened.lastSeq).toBe(0);
		expect(
			readSince(runtime.queries, SESSION, { epoch: opened.epoch, seq: 1 }),
		).toEqual({ ok: false, reset: "epoch_changed" });
	});
	...
	test("an existing journal keeps its epoch across reopen", () => {
		const opened = runtime.journal.open(init());
		seed(runtime, 2);
		expect(new ChatJournal(runtime.queries).open(init()).epoch).toBe(
			opened.epoch,
		);
	});
});
...
describe("readPage", () => {
	...
	test("pages backwards from newest and reports the next bound", () => {
		...
		const newest = readPage(runtime.queries, SESSION, { limit: 2 });
		...
		const older = readPage(runtime.queries, SESSION, {
		...
		const oldest = readPage(runtime.queries, SESSION, {
	...
	test("returns an empty page for an empty journal", () => {
		runtime.journal.open(init());
		expect(readPage(runtime.queries, SESSION, { limit: 10 })).toEqual({
			ok: true,
			envelopes: [],
			nextBefore: null,
		});
	...
	test("signals epoch_changed for a stale-epoch bound", () => {
		...
		seed(runtime, 2);
		expect(
			readPage(runtime.queries, SESSION, {
