...
import type { FakeHarnessScript } from "./harness/fake/fakeHarness";
import { FakeHarness } from "./harness/fake/fakeHarness";
import type { ChatRuntime } from "./index";
import { readSince } from "./journal/replay";
import type { HarnessRegistry } from "./sessions/registry";
import type { Schedule, Sink } from "./stream/subscriptions";
...
export function journalEnvelopes(
	...
	if (!session) return [];
	const replay = readSince(runtime.queries, sessionId, {
		epoch: session.epoch,
		seq: 0,
	});
